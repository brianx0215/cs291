...
function createStairs() {
	...
	for(let i = 0; i < 6; i++){
		...
		stepMesh.position.y = i * (verticalStepHeight + stepThickness) + 0.5 * verticalStepHeight;	// half of height: put it above ground plane
		...
		stepMesh.position.y = (i + 1) * (verticalStepHeight + stepThickness) - 0.5 * stepThickness;
		// Push step forward by half the depth, minus half the vertical step's thickness
		stepMesh.position.z = (horizontalStepDepth - stepThickness) * (0.5 + i);
		scene.add( stepMesh );
	}
